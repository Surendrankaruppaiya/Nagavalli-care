...
const Appointment = () => {
  ...
  const fetchFeedbacks = async () => {
    try {
      const res = await axios.get(`${backendUrl}/api/feedback/${docId}`);
    ...
    <div>
      ...
      <div className='flex flex-col sm:flex-row gap-4'>
        ...
        <div className='flex-1 border border-[#ADADAD] rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0'>
          ...
          {/* Live Chat & Feedback Buttons */}
          <div className="flex justify-end gap-4 mt-4">
            <button
              onClick={() => navigate(`/ChatPage`)}
              className="bg-green-600 text-white px-6 py-2 rounded-full hover:bg-green-700 transition-all text-sm"
            >
              💬 Live Chat
            </button>
            <button
              onClick={() => navigate(`/FeedbackPage/${docInfo._id}`)}
      ...
      {/* Feedback Section */}
      <div className='mt-8 px-4 sm:px-72'>
